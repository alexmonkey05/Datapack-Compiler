...
# 2025/01/30 3.0.5b
- `/$$(command)`와 같이 명령어의 처음부터 매크로를 사용하면 실행이 안 되던 에러 해결
- 매크로를 컴파일하는 과정에서의 속도 향상
# 2024/02/03 3.0
- `del(data)`라는 코드가 있으면 인게임에서 많은 렉이 걸리고 튕기는 현상 해결
- 배열을 만들 때 같은 배열을 두번 만드는 현상 해결
# 2025/02/04 3.0.1
...
- 테스트 케이스 모두 통과
- `README.md` 수정
# 2025/09/08 3.7 GUI 모드 기능 추가
- GUI에서 마지막에 컴파일 한 값을 다시 켰을 때 기억한다
# 2025/10/28 3.7.1
- `is_module()` 사용 시 에러가 나던 버그 수정
- 라이센스 표시 추가
...
- `transform.py`의 변환 구조를 Lark `Transformer` 기반에서 top-down 방문 방식으로 변경
  - 블록 진입/종료 시 지역 스코프를 push/pop하도록 변경
  - 함수 매개변수를 함수 스코프의 지역변수로 처리
  - 같은 깊이에서 같은 이름의 변수를 다시 선언하면 에러가 나도록 변경
  - 함수명과 같은 이름의 변수 선언은 가능하도록 유지
